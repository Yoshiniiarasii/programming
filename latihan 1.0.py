...
# --- 1. PAGE CONFIG ---
st.set_page_config(page_title="DEVELOPMENT OF A SURVEY LOT INFORMATION SYSTEM", layout="wide")

# --- 2. GLOBAL STYLING & ACCESSIBILITY ---
st.markdown("""
    <style>
    /* Background: Housing Area Theme */
    .stApp { 
    ...
    label, p, span, h1, h2 { color: white !important; }
...
if "user_db" not in st.session_state:
    st.session_state["user_db"] = {
        "1": {"name": "YOSHINIARASII", "pw": "MYadmin123"},
        "2": {"name": "SAHTHANAAH", "pw": "MYadmin123"},
...
# --- 6. MAIN DASHBOARD ---
    ...
    with st.sidebar:
        ...
        poly_color = st.color_picker("Warna Poligon", "#A855F7")
        ...
        with st.expander("🔑 Tukar Kata Laluan"):
            current_p = st.text_input("Katalaluan Lama", type="password")
            next_p = st.text_input("Katalaluan Baru", type="password")
            if st.button("Simpan"):
                if current_p == user["pw"]:
                    st.session_state["user_db"][st.session_state["uid"]]["pw"] = next_p
                    st.success("Tukar Berjaya!")
                else: st.error("Lama salah.")
    ...
    # --- LOGO SECTION ---
    # Put your local file name here: 'POLI LOGO.png'
    st.image("POLI LOGO.png", width=250)
    
    st.markdown("<h1 style='color:#a855f7 !important;'>SISTEM SURVEY LOT</h1><p>Jabatan Kejuruteraan Awam</p>", unsafe_allow_html=True)

    col1, col2 = st.columns([1, 3])
    with col1: epsg = st.text_input("🌏 Kod EPSG:", "4390")
    with col2: uploaded_file = st.file_uploader("Upload CSV (STN, E, N)", type=['csv'])
    ...
    if uploaded_file:
        ...
        popup_html = f"<b>Polygon Coordinates:</b><br>{coords_str}"
        
        folium.GeoJson(
            gdf_wgs, 
            style_function=lambda x: {'color': poly_color, 'weight': 3, 'fillOpacity': 0.1},
            tooltip="Click to view coordinates"
        ...
        for i in range(len(df)):
            ...
            folium.Marker(
                [p1_geo.y, p1_geo.x], 
                icon=folium.DivIcon(html=f'<div style="color:white; font-weight:bold; background:red; border-radius:50%; width:{st_size}px; height:{st_size}px; line-height:{st_size}px; text-align:center; border:2px solid white;">{df.iloc[i]["STN"]}</div>')
            ).add_to(fg_stn)
            ...
            folium.Marker([lbl_gdf.iloc[0].geometry.y, lbl_gdf.iloc[0].geometry.x], icon=folium.DivIcon(html=f'<div style="color:{poly_color}; font-size:{text_size}pt; font-weight:bold; text-shadow:1px 1px 3px black; width:100px;">{brg}</div>')).add_to(fg_lbl)
            folium.Marker([lbl_gdf.iloc[1].geometry.y, lbl_gdf.iloc[1].geometry.x], icon=folium.DivIcon(html=f'<div style="color:white; font-size:{text_size}pt; font-weight:bold; text-shadow:1px 1px 3px black; width:100px;">{dist}</div>')).add_to(fg_lbl)
